...
class CMModules extends CObject{
  ...
  private $brosephinaCoreModules = array('CBrosephina', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $brosephinaCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  public function ReadAndAnalyse() {
    $src = Brosephina_INSTALL_PATH.'/src';
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isBrosephinaCore']   = in_array($rc->name, $this->brosephinaCoreModules);
      $details['isBrosephinaCMF']    = in_array($rc->name, $this->brosephinaCMFModules);
